...
export const Header: React.FC<HeaderProps> = ({
  ...
  const headerView = <View style={isMainHeader ? styles.darkOverlay : {
    flex: 1
  }}>
      ...
      height: headerContentHeight
        ...
        <View style={isCenteredLayout ? styles.leftContainerAbsolute : styles.leftContainerCustom}>
          {onBackPress && <TouchableOpacity onPress={onBackPress} style={styles.backBtn} activeOpacity={0.7}>
              <MaterialCommunityIcons name={backIconName} size={28} color={textColor} />
            </TouchableOpacity>}
        </View>

        <View style={isCenteredLayout ? styles.titleContainerAbsolute : styles.titleContainerCustom}>
          {children ? children : title ? onTitlePress ? <TouchableOpacity onPress={onTitlePress} activeOpacity={0.8}>
      ...
      {isMainHeader && showSearch && <View style={styles.searchContainer}>
          <TouchableOpacity style={styles.searchPill} activeOpacity={onSearchPress ? 0.9 : 1} onPress={onSearchPress} disabled={!onSearchPress}>
            <MaterialCommunityIcons name="magnify" size={22} color="rgba(255,255,255,0.7)" style={{
          marginLeft: 16
        }} />
          ...
          {searchRightElement && <View style={styles.searchRightWrapper}>
              {searchRightElement}
            </View>}
        </View>}
    </View>;
  return <View style={[styles.container, {
    ...
    height: actualHeight,
    overflow: 'hidden'
  }, showShadow && styles.shadow, style]}>
      {isMainHeader ? <View style={{
      flex: 1
    }}>
          {}
