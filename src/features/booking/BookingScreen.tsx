...
import { View, Text, StyleSheet, TouchableOpacity, Alert, TextInput, ScrollView, Platform, ToastAndroid } from "react-native";
...
const getFareForDistance = (distanceKm: number, busType: "AC" | "Non-AC", isNCR: boolean, currentFareConfig: typeof fareConfig): {
  fare: number;
  slab: any;
} => {
  ...
  const fare = busType === "AC" ? applicableSlab.acFare : applicableSlab.nonACFare;
  return {
    fare,
    slab: applicableSlab
  };
...
const BusTypeSelector = React.memo(({
  ...
  compact?: boolean;
}) => <View style={[styles.inputSection, compact && styles.customInputSection, {
  marginBottom: compact ? 0 : 8
}]}>
      <Text style={[styles.inputLabel, compact && styles.customInputLabel]}>Bus Type</Text>
      <View style={styles.typeRow}>
        {(["AC", "Non-AC"] as const).map(type => <TouchableOpacity key={type} style={[styles.typeBtn, compact && styles.customTypeBtn, type === "AC" && {
      paddingHorizontal: moderateScale(10),
      minWidth: moderateScale(40)
    }, busType === type && (type === "AC" ? styles.typeBtnActive : styles.typeBtnActiveNonAC)]} onPress={() => onTypeChange(type)}>
            <Text style={[styles.typeBtnText, compact && {
...
const QuantitySelector = React.memo(({
  ...
  onQtyChange: (n: number) => void;
      ...
      <View style={styles.qtyRow}>
        {[1, 2, 3].map(n => <TouchableOpacity key={n} style={[styles.qtyBtn, qty === n && styles.qtyBtnActive]} onPress={() => onQtyChange(n)}>
            <Text style={[styles.qtyBtnText, qty === n && styles.qtyBtnTextActive]}>
              {n}
            </Text>
          </TouchableOpacity>)}
      </View>
...
const FareDisplay = React.memo(({
  ...
  useEffect(() => {
    opacityVal.value = withTiming(showDiscount ? 1 : 0, {
      duration: 350
    });
  ...
  return <View style={styles.fareRow}>
        <View style={{
      flex: 1
    }}>
          ...
          <Animated.View style={[styles.priceRow, animatedStyle]}>
            ...
            alignItems: "center"
                  ...
                  <TextInput style={[styles.newPrice, {
              minWidth: moderateScale(40),
              padding: 0
            }]} value={manualTotal} onChangeText={onManualChange} onBlur={onBlur} onSubmitEditing={onBlur} keyboardType="numeric" autoFocus selectTextOnFocus />
                </View> : <Text style={styles.newPrice}>₹{finalFare.total}</Text>}
            </TouchableOpacity>
          </Animated.View>
        </View>
        <Animated.View style={[{
      ...
      alignItems: "center"
          ...
          {finalFare.toll ? <View style={styles.tollBadge}>
...
export const BookingScreen = ({
  ...
  const [timeLeftForLogic, setTimeLeftForLogic] = useState(180);
  ...
  const saveCustomHistory = useCallback(async (item: CustomHistoryItem) => {
    try {
      ...
      list = list.filter(x => !(x.route.toLowerCase() === item.route.toLowerCase() && x.source.toLowerCase() === item.source.toLowerCase() && x.dest.toLowerCase() === item.dest.toLowerCase() && x.fare === item.fare && x.busType === item.busType));
      ...
      setSecretTapCount(prev => {
        ...
        if (nextCount === 7) {
          setIsAdminUnlocked(adminPrev => {
            ...
            Haptics.notificationAsync(nextState ? Haptics.NotificationFeedbackType.Success : Haptics.NotificationFeedbackType.Warning);
            if (Platform.OS === "android") {
              ToastAndroid.show(nextState ? "Developer Mode Unlocked!" : "Developer Mode Locked", ToastAndroid.SHORT);
            } else {
              Alert.alert("Developer Mode", nextState ? "Developer Mode Unlocked!" : "Developer Mode Locked");
            }
  ...
  const calculateAutoFare = useCallback(() => {
    if (!routeSearch || !sourceSearch || !destSearch || !selectedFullRouteId) {
      return {
        fare: 0,
        slab: fareConfigState.delhiSlabs[0],
        isValid: true
      };
    }
    const foundRoute = dbRoutes.find(r => r.id?.toLowerCase() === selectedFullRouteId?.toLowerCase() || r.name?.toLowerCase() === selectedFullRouteId?.toLowerCase());
    if (!foundRoute) {
      return {
        fare: 0,
        slab: fareConfigState.delhiSlabs[0],
    ...
    const distanceKm = stopCount * 0.6;
    const {
      fare,
      slab
    } = getFareForDistance(distanceKm, busType, isInterstate, fareConfigState);
    return {
      fare,
      slab,
      isInterstate,
  ...
  const getFinalFare = useCallback(() => {
    ...
    return {
      ...currentFare,
      isInterstate,
      originalTotal: subTotal.toFixed(1),
  ...
  const handleBuy = useCallback(() => {
    if (bookingMode === "regular") {
      ...
      const ticketData = {
        ...
        qty: qty,
        busType: busType,
        fare: Number(finalFare.total),
        baseFare: currentFare.fare,
        finalFare: finalFare.total,
        originalTotal: finalFare.originalTotal,
      ...
      const ticketData = {
        ...
        qty: qty,
        busType: busType,
        fare: Number(discountedTotal.toFixed(1)),
        baseFare: customFare,
        finalFare: discountedTotal.toFixed(1),
        originalTotal: baseFareTotal.toFixed(1),
        total: discountedTotal.toFixed(1),
  ...
  }, [bookingMode, routeSearch, sourceSearch, destSearch, customRoute, customSource, customDest, customFare, qty, busType, getCurrentFare, getFinalFare, navigation, timeLeft, saveCustomHistory]);
  ...
  return <PortalProvider>
      <Screen noPadding ignoreTopSafe style={styles.container}>
        {}
        <Header title="Buy tickets" centerTitle={true} onBackPress={() => navigation.goBack()} titleStyle={{
        fontSize: 22
      }} onTitlePress={handleSecretTap} />

        {}
        <View style={[styles.timerContainer, bookingMode === "custom" && {
        ...
        <View style={styles.mainLayout}>
          <ScrollView style={styles.contentScroll} contentContainerStyle={[styles.contentScrollContainer, bookingMode === "custom" && {
          paddingTop: moderateScale(4),
          paddingBottom: moderateScale(8)
        }]} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" scrollEventThrottle={16}>
            <View style={[styles.card, bookingMode === "custom" && {
            paddingTop: moderateScale(12)
              ...
              {bookingMode === "regular" ? <>
                  ...
                  <View style={styles.inputSection}>
                    ...
                    <SearchableDropdown ref={sourceDropdownRef} data={sourceDropdownData} value={sourceSearch} onChangeText={setSourceSearch} onSelect={(item: any) => {
                  ...
                  {}
                  {customHistory.length > 0 && <View style={styles.historySection}>
                      ...
                      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.historyChipsRow}>
                        {customHistory.map((item, idx) => <TouchableOpacity key={idx} style={styles.historyChip} onPress={() => {
                  ...
                  }} onBlur={() => setCustomRoute(prev => prev.trim())} autoCapitalize="characters" />
                    </View>
                  </View>
                  ...
                  position: "relative"
                      ...
                      <View style={[styles.inputBox, styles.customInputBox, customErrors.source && styles.inputBoxError, {
                    marginBottom: 6
                  }]}>
                        <View style={styles.inputIcon}>
                ...
                }} contentContainerStyle={styles.fareChipsRow} keyboardShouldPersistTaps="handled">
                      {[5, 10, 15, 20, 25].map(f => <TouchableOpacity key={f} style={[styles.fareChip, customFare === f && !isCustomInputActive && styles.fareChipActive]} onPress={() => {
                    setCustomFare(f);
                    setCustomFareInput("");
              ...
              {}
              <BusTypeSelector busType={busType} onTypeChange={type => {
              setBusType(type);
              setIsManualFare(false);
              setManualTotal("");
              setManualBaseFare(0);
